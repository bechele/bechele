...
$cfgdir=~s/\/$//;                                     # remove trailing / to keep path clean
...
sub put_one_move {
  ...
  my $pos2=int($joy_content[5]/($servores/($$servosettings[$secondservo][4] - $$servosettings[$secondservo][3]))+$$servosettings[$secondservo][3]);
  my $pcaref=[0,$pos1,0,$pos2];
  $netstream[$currentservo]=$pos1;
  $netstream[$secondservo]=$pos2;
  if ($sendtopca){
  ...
  print "\fX-pos(0-$servores)	|Y-Pos(0-$servores)	|Joystick X 	|Joystick Y	|Servo $currentservo	|Servo $secondservo\n";
  print "------------------------------------------------------------------------------------------------\n";
